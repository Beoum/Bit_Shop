				...
				<c:if test="${param.menu == 'manage' }">
					<c:choose>
						<c:when test="${list[i].proTranCode == '0' }">
							<td align="left"><a href="/product/updateProductView?prodNo=${list[i].prodNo }&menu=${param.menu}">${list[i].prodName }</a></td>
						</c:when>
						<c:otherwise>
							<td align="left"><a href="/product/getProduct?prodNo=${list[i].prodNo }&menu=${param.menu}">${list[i].prodName }</a></td>
						</c:otherwise>
					</c:choose>	
				</c:if>
				...
				<c:if test="${param.menu == 'manage' }">
					<c:choose>
						<c:when test="${list[i].proTranCode.charAt(0) == '1'.charAt(0) }">
							구매완료<a href="/purchase/updateTranCodeByProd?prodNo=${list[i].prodNo }&tranCode=2&currentPage=${resultPage.currentPage}&searchKeyword=${param.searchKeyword}&menu=${param.menu}">배송하기</a>
						</c:when>
